...
public class OI {
  ...
  private OI() {
    ...
    buttonRightBumper.whileHeld(
        new IntakeMove(Constants.Values.INTAKE_IN, true)/* new ShooterStream(Constants.Values.SHOOTER_RPM) */);
    buttonLeftBumper.whileHeld(new IntakeMove(Constants.Values.INTAKE_EJECT, false));// 7.5 /*new
                                                                                     // ShooterStreamAutoTarget(Constants.Values.SHOOTER_RPM)*/
     ...
     * buttonRightBumper2.whileHeld(new IntakeMove(Constants.Values.INTAKE_IN,
     * true)); buttonLeftBumper2.whileHeld(new
     * IntakeMove(Constants.Values.INTAKE_EJECT, false));
     */
   ...
   *
   * public double getGamepad2Axis(int portNum) { axisPos =
   * gamepad2.getRawAxis(portNum); if (Math.abs(axisPos) < 0.05) { axisPos = 0; }
   * return axisPos; }
   */
